...
import { projectsData } from "../ulits/data";
...
const Projects = () => {
  ...
  const handleNext = () => {
    setTimeout(() => {
      setStartIndex((prevIndex) =>
        Math.min(
          prevIndex + (isLaptopScreen ? 1 : 2),
          projectsData.length - (isLaptopScreen ? 1 : 2)
        )
      );
  ...
  const handlePrev = () => {
    setTimeout(() => {
      setStartIndex((prevIndex) =>
        Math.max(prevIndex - (isLaptopScreen ? 1 : 2), 0)
      );
    }, 200); // Adjust the duration to match the animation duration
  };
  ...
  for (let i = 0; i < numPages; i++) {
    dots.push(
      <div
        key={i}
        className={`dot ${
          i === startIndex / (isLaptopScreen ? 1 : 2) ? "active" : ""
        }`}
      />
    );
  }
  ...
  return (
    <div id="projects" className="projects-container">
      <div className="projects-list">
        {projectsData
          .slice(startIndex, startIndex + (isLaptopScreen ? 1 : 2))
          .map((project, index) => (
      ...
      >
        <div
          className={`control-arrow ${startIndex === 0 ? "disabled" : ""}`}
          onClick={handlePrev}
        >
          &#8249;
        </div>
        ...
        <div
          className={`control-arrow ${
            startIndex + (isLaptopScreen ? 1 : 2) >= projectsData.length
              ? "disabled"
              : ""
          }`}
          onClick={handleNext}
